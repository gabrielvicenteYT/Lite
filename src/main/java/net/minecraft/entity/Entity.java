...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            if (Lite.EVENT_BUS.<Event>call(new SafeWalkEvent()).isCancelled() || flag)
                {
                double d6;
                ...
                if (callEvent) {
                    Lite.EVENT_BUS.call(new WalkOnEdgeEvent());
                }
            }
            ...
            stepEvent.setType(Event.Type.PRE);

            Lite.EVENT_BUS.call(stepEvent);

            if (stepEvent.getStepHeight() > 0.0F && flag1 && (d3 != x || d5 != z))
            {
                double d11 = x;
                double d7 = y;
                double d8 = z;
                ...
                StepEvent stepEventPost = new StepEvent(this, 1F + stepHeight);
                stepEventPost.setType(Event.Type.POST);

                Lite.EVENT_BUS.call(stepEventPost);
            }
    ...
    {
        StrafeEvent strafeEvent = new StrafeEvent(strafe, friction, forward);

        if (!Lite.EVENT_BUS.<Event>call(strafeEvent).isCancelled()) {
